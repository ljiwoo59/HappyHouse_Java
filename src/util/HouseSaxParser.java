...
import happyhouse.model.dto.HouseDeal;
import happyhouse.model.dto.HouseInfo;
...
public class HouseSaxParser {
	...
	private void loadData() {
		...
		try{
			...
			for (List<HouseDeal> houseDeal : deals.values()) {
				for (HouseDeal houseDeal2 : houseDeal) {
					HouseInfo info = houseInfo.get(houseDeal2.getDong() + houseDeal2.getAptName());
					if (info != null)
						houseDeal2.setImg(info.getImg());
				}
			}
		}catch(Exception e){
			e.printStackTrace();
